...
@section('title', 'داواکارییەکانی مامۆستا')

@section('content')

<div class="page-header">
    <div>
        <h1>داواکارییەکانی مامۆستا</h1>
        <div class="breadcrumb">ئەو داواکاریانەی لەلایەن مامۆستایانەوە نێردراون بۆ دروستکردنی پرۆفایل</div>
    </div>
</div>

<div class="card">
    <form method="GET" action="{{ route('admin.teacher-requests.index') }}" class="filter-bar">
        ...
        @if(request()->filled('status'))
            <div class="form-group" style="flex: 0;">
                <a href="{{ route('admin.teacher-requests.index') }}" class="btn btn-ghost" title="پاککردنەوە" style="height: 42.5px;">
                    ...
                    <tr>
                        ...
                        <td style="text-align:left;">
                            <div class="d-flex gap-2" style="justify-content:flex-end;">
                                @if($req->status == 'pending')
                                    <form action="{{ route('admin.teacher-requests.status', $req) }}" method="POST">
                                        @csrf
                                ...
                                @endif
                                <form action="{{ route('admin.teacher-requests.destroy', $req) }}" method="POST" class="confirm-action" data-confirm="دڵنیایت لە سڕینەوەی ئەم داواکارییە؟">
                                    @csrf @method('DELETE')
